...
| `--test-confidence <path>` | Path to `test-confidence-review` JSON evidence | No |
| `--test-confidence-review <path>` | Path to qualitative test-confidence review notes | No |
...
If `--test-confidence` is provided, read it as an auxiliary validation checkbox. Treat it as passing only when `passed: true` and `required_checks_passed: true`; treat missing or malformed evidence as unavailable. If `--test-confidence-review` is provided, use it only to summarize test-quality concerns or to lower confidence when the tests appear shallow, implementation-mirroring, or low-value.
...
If the test-confidence checkbox is failed or unavailable, keep any `BLOCKED` verdict from the rules above; otherwise prefer `NEEDS_WORK` over `SHIP` only when the validation evidence materially lowers confidence in the implemented requirements. Do not add a new JSON field or change the human report shape for this signal.
